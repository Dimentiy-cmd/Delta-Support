...
try:
    import certifi  # Use certifi CA bundle to avoid local issuer issues
...
class RemnaWaveAPI:
    ...
    async def __aenter__(self):
        ...
        logger.debug('Подключение к Remnawave: base_url={}, тип: {}', self.base_url, conn_type)
            ...
            try:
                ...
                async with self.session.request(method, **kwargs) as response:
                    ...
                    if response.status >= 400:
                        ...
                        log('API Error {}: {}', response.status, error_message)
                        log('Response: {}', response_text[:500])
                ...
                if attempt < max_retries:
                    ...
                    logger.warning(
                        'Request failed on retry {}/{} after {}s. method={}, endpoint={}, error={}',
                        attempt + 1, max_retries, delay, method, endpoint, e,
                    )
    ...
    async def _resolve_user_id(self, identifier: Any) -> int | None:
        """Resolve legacy uuid/shortUuid/numeric id to Remnawave 3+ numeric user id."""
    ...
    async def create_user(
        ...
        try:
            ...
            if error_code == 'A039' and 'externalSquadUuid' in data:
                ...
                logger.error('POST /api/users FAILED — full payload: {}', data)
                raise
        user = self._parse_user(response['response'])
        logger.info(
            'POST /api/users response: uuid={}, hwidDeviceLimit={}',
            user.uuid,
    ...
    async def get_user_by_uuid(self, uuid: str) -> RemnaWaveUser | None:
        resolved_id = await self._resolve_user_id(uuid)
        try:
            if resolved_id is not None:
                response = await self._make_request('GET', f'/api/users/{resolved_id}')
            else:
                response = await self._make_request('GET', f'/api/users/by-short-uuid/{uuid}')
            user = self._parse_user(response['response'])
            return await self.enrich_user_with_happ_link(user)
        except RemnaWaveAPIError as e:
            if resolved_id is None and e.status_code in (400, 404):
                try:
                    response = await self._make_request('GET', f'/api/users/{uuid}')
                    user = self._parse_user(response['response'])
                    return await self.enrich_user_with_happ_link(user)
                except RemnaWaveAPIError as legacy_e:
                    if legacy_e.status_code == 404:
                        return None
                    raise
    ...
    async def find_users(
        ...
        *,
        ...
        while True:
            ...
            data = response.get('response') or {}
            users = [self._parse_user(item) for item in data.get('users', [])]
    ...
    async def update_user(
        ...
        resolved_id = await self._resolve_user_id(uuid)
        data = {'id': resolved_id} if resolved_id is not None else {'uuid': uuid}
        ...
        try:
            ...
            if error_code == 'A039' and 'externalSquadUuid' in data:
                ...
                logger.error('PATCH /api/users FAILED — full payload: {}', data)
                raise
        user = self._parse_user(response['response'])
        logger.info(
            'PATCH /api/users response: uuid={}, hwidDeviceLimit={}',
            uuid,
            user.hwid_device_limit,
        )
        return await self.enrich_user_with_happ_link(user)
    ...
    async def delete_user(self, uuid: str) -> bool:
        identifier = await self._user_endpoint_identifier(uuid)
        await self._make_request('DELETE', f'/api/users/{identifier}')
        return True

    async def enable_user(self, uuid: str) -> RemnaWaveUser:
        identifier = await self._user_endpoint_identifier(uuid)
        response = await self._make_request('POST', f'/api/users/{identifier}/actions/enable')
        user = self._parse_user(response['response'])
        return await self.enrich_user_with_happ_link(user)

    async def disable_user(self, uuid: str) -> RemnaWaveUser:
        identifier = await self._user_endpoint_identifier(uuid)
        response = await self._make_request('POST', f'/api/users/{identifier}/actions/disable')
        user = self._parse_user(response['response'])
        return await self.enrich_user_with_happ_link(user)

    async def reset_user_traffic(self, uuid: str) -> RemnaWaveUser:
        identifier = await self._user_endpoint_identifier(uuid)
        response = await self._make_request('POST', f'/api/users/{identifier}/actions/reset-traffic')
        user = self._parse_user(response['response'])
        return await self.enrich_user_with_happ_link(user)
    ...
    async def revoke_user_subscription(
        ...
        identifier = await self._user_endpoint_identifier(uuid)
        response = await self._make_request('POST', f'/api/users/{identifier}/actions/revoke', data)
        user = self._parse_user(response['response'])
        return await self.enrich_user_with_happ_link(user)
    ...
    async def get_user_accessible_nodes(self, uuid: str) -> list[RemnaWaveAccessibleNode]:
        ...
        try:
            identifier = await self._user_endpoint_identifier(uuid)
            response = await self._make_request('GET', f'/api/users/{identifier}/accessible-nodes')
    ...
    async def restart_node(self, uuid: str) -> bool:
        await self._make_request('POST', f'/api/nodes/{uuid}/actions/restart')
        return True
    ...
    async def get_user_devices(self, user_uuid: str) -> dict[str, Any]:
        identifier = await self._user_endpoint_identifier(user_uuid)
        try:
            response = await self._make_request('GET', f'/api/hwid/devices/{identifier}')
            return response['response']
        except RemnaWaveAPIError as e:
            if e.status_code == 404:
    ...
    async def get_user_devices_all(self, user_uuid: str) -> dict[str, Any]:
        ...
        try:
            while True:
                response = await self._make_request(
                    'GET', f'/api/hwid/devices/{identifier}', params={'start': start, 'size': page_size}
                )
                data = response.get('response', {'devices': [], 'total': 0})
                devices = data.get('devices', [])
                total = data.get('total', 0)
    ...
    async def reset_user_devices(self, user_uuid: str) -> bool:
        try:
            resolved_id = await self._resolve_user_id(user_uuid)
            if resolved_id is not None:
                ...
                if isinstance(payload, dict):
                    return int(payload.get('total') or len(payload.get('devices') or [])) == 0
                return True
            ...
            for device in devices:
                ...
                if device_hwid:
                    try:
                        ...
                        logger.error('Ошибка удаления устройства: device_hwid={}, error={}', device_hwid, device_error)
                        failed_count += 1

            return failed_count < len(devices) / 2

        except Exception as e:
            logger.error('Ошибка при сбросе устройств: {}', e)
            return False

    async def remove_device(self, user_uuid: str, device_hwid: str) -> bool:
        try:
            resolved_id = await self._resolve_user_id(user_uuid)
            delete_data = {'userId': resolved_id, 'hwid': device_hwid} if resolved_id is not None else {'userUuid': user_uuid, 'hwid': device_hwid}
            ...
            if isinstance(payload, dict) and isinstance(payload.get('devices'), list):
                return all(str(item.get('hwid') or '') != str(device_hwid) for item in payload.get('devices') or [])
            return True
        except Exception as e:
            logger.error('Ошибка удаления устройства: device_hwid={}, error={}', device_hwid, e)
            return False
    ...
    async def encrypt_happ_crypto_link(self, link_to_encrypt: str) -> str | None:
        try:
            ...
            logger.warning('Не удалось зашифровать happ ссылку: {}', e.message)
            return None
        except Exception as e:
            logger.warning('Ошибка при шифровании happ ссылки: {}', e)
            return None
    ...
    def _parse_user(self, user_data: dict) -> RemnaWaveUser:
        ...
        return RemnaWaveUser(
            uuid=user_data.get('uuid') or user_data.get('shortUuid') or str(user_data.get('id') or ''),
...
async def test_api_connection(api: RemnaWaveAPI) -> bool:
    try:
        ...
        logger.warning('API connection test failed: {}', e)
        return False